...
| **MQ-P0-04** | **GBC-0026** — `GO-EXTERNAL-VESTIBULE-01` is `DEFERRED` yet its Airlock/Vestibule distinction is load-bearing for three live mission authorizations | A `DEFERRED` record is doing current authoritative work. Status and use disagree | **DONE 2026-09-05** — Product Owner chose promotion: [`GOV-P-005`](../records/GOV-P-005.md) v1.0. The record's `DEFERRED` status is correct as it stands and was left alone, because it describes the unbuilt admission *workflow*; the *distinction* now sits at the `GOV-P` layer where live authorizations can cite it. Verification found four `RUN` mission authorizations invoking it, not three |
