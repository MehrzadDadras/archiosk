...
**Status:** Implemented. **Ground truth:** `services/case_workspace.py` in `C:\Archiosk\Research\archiosk` (backend repo), re-verified this session at 4,594 lines. Test suite: `tests/test_foundation_batch_a.py` through `test_foundation_batch_k.py`, stdlib `unittest`, 166 passing tests as of the last full run this session. Every anchor below was checked directly against current code, not recalled from an earlier round — Batch K was added mid-engagement and could have shifted line numbers, so nothing here is assumed stable across time; re-verify before citing in future work if material time has passed.
