...
**Capability-level status, precise, not blanket:** only the first leg — **Private → explicit Share → Shared — is implemented** (Case-visibility, Private + Explicit Share Only tranche; see `current/kernel-object-model.md`'s "Case visibility (PRIVATE/SHARED)" entry for the code/test anchor). Everything from Shared onward — the collaboration threshold, Shared→Private retraction, Collaborative status itself, Archive, `CaseLock`, Derive/Copy/Adopt, and publication anchoring — remains specified but unbuilt, exactly as below. This document's overall "Specified, not implemented" status line (top of file) applies to everything except that one implemented leg; it is not a blanket claim, and the reverse is equally true — implementing Private→Shared does not mean any of the rest is implemented.
