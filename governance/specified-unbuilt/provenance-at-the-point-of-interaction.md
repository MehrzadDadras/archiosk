...
### C.1 Rebuild start state — the canvas is not recoverable from git

`routes/spike_arm_b.py` and `templates/spike/arm_b_canvas.html` were built for
the control arm and deleted before commit, per this repository's discipline that
exploratory apparatus stays out of version control. Verified against the tree at
`f0a12ce` rather than inferred from its commit message: there is no
`templates/spike/` entry, no `spike_arm_b` module, and no canvas blueprint among
`app.py`'s registrations. The branch name `spike/multi-surface-canvas` names the
**experiment**, not a surviving surface.

Any Canvas UI work therefore starts from this specification and from
`governance/proposals/surface-vs-substrate-interaction-grammar.md`, not from
recoverable code. The 30 `getElementById` bindings named above are the first
question a rebuild must answer, not the last — and section B's finding stands as
the acceptance bar: a rebuilt canvas must carry **record-grounded** links
(`AnalysisRun.source_ids`), never links parsed from statement text, because a
citation that can lie is worse than a UUID.
