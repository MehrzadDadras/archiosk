...
why); rows 31-33 (`CLAUDE-SPREADSHEET-SOURCE-ELIGIBILITY-01`) are
accurate as of commit `c4bcbd4`, deployed to production 2026-08-17,
hermetically verified only (targeted lane: 191 passed; full suite did
not complete this round - see row 33) - the derived post-fix North
Bayview Reconcile counts in row 32 are NOT yet live-reverified. If the
...
| 32 | **Direct inspection of the real, frozen North Bayview specimen found SEVEN `.xlsx` files, not the one workbook the governing prompt's own framing named.** `C:\Archiosk\Samples\north-bayview\data-room` contains `data-room-index-v2.xlsx`, `f-11-design-conformance-matrix-template.xlsx`, `f-12-deviations-and-departures-register-template.xlsx`, `north-bayview-owner-reference-space-program-v1.0.xlsx`, `rfi-log-v3-annotated.xlsx`, `rfi-log-v4.xlsx`, `financial-model-template-v1.xlsx`, plus one `.svg` — exactly accounting for the previously-observed "8 skipped/ineligible." None of these 7 workbooks were ever registered as a Source under any prior establishment/Reconcile round (the extension gate excluded all of them identically, not just the one named workbook), so row 31's fix makes all 7 newly eligible/`new` simultaneously, not one — a materially more consequential result than the prompt's own "the workbook" framing suggested. **Not yet re-verified against the live corpus this round**: a live Reconcile re-run against production (needed to get the actual, current, authoritative post-fix counts, not a derived expectation) requires either live-browser authentication (blocked this session — see row 30) or a narrowly-scoped read-only remote diagnostic that was not attempted before this round's own explicit "commit and finish" instruction. The `7 new / 1 ineligible` expectation above is code-and-corpus-grounded (extension-eligibility logic plus a direct listing of the real files) but is a derived expectation, not a directly re-observed live result — flagged explicitly per this stage's own "recalculate the truth... rather than forcing an expected count" instruction, which this row deliberately does NOT claim to have satisfied yet. | High (file inventory) / Medium (derived post-fix Reconcile counts, not live-reverified) | open (live re-verification still needed) | `CLAUDE-SPREADSHEET-SOURCE-ELIGIBILITY-01`, 2026-08-17 |
| 33 | **A real, severe full-suite hang recurred this session, independent of any one code change.** A full `pytest` run (commit `c4bcbd4`'s own changes) stalled for 2.5+ hours past its normal ~20min duration with the process still alive and consuming CPU, then a second verbose diagnostic run was stopped deliberately (not hung) after ~40s per Product Owner instruction to stop chasing it and commit. This matches `CLAUDE.md`'s own documented precedent exactly ("a single background test run to take 8.5 hours... a real, un-mocked `ingest_upload` call hung against a live API") — a known, recurring, NOT-yet-eliminated class of risk in this repo's test suite, not a one-off. Neither hang was root-caused to a specific test this session (output buffering on the first run made the exact stall point unrecoverable; the second run was killed too early by design to reproduce it). Windows `ps -W`'s `PID` column is NOT a real Windows PID for `taskkill` — the `WINPID` column is; using the wrong one silently fails ("process not found") without indicating which column was needed. | High (hang recurrence, tooling gotcha) / Low (root cause, not yet isolated) | open (full-suite confirmation for commit `c4bcbd4` still outstanding; root cause of the hang still unisolated) | `CLAUDE-SPREADSHEET-SOURCE-ELIGIBILITY-01`, 2026-08-17 |
