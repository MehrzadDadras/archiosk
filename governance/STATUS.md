...
`services/case_workspace.py` (backend repo), 10 Foundation Batches plus the `promote_requirement_item()` bridge and Case-visibility (Private/Shared) tranches, 209 passing `unittest` tests. Full detail and code anchors: `current/kernel-object-model.md`. In one sentence: `Source`, `Requirement`, `Finding`, `Relationship`, `Case`, `ReviewerValidation`, `Disposition`, `RequirementAdjudication`, `Snapshot`, `Supersession`, `TemporalObligation`, `AnalysisRun`/`AnalysisTrigger`, `GovernanceLog`, the `promote_requirement_item()` bridge, and Case visibility (`CASE_VISIBILITY_PRIVATE`/`CASE_VISIBILITY_SHARED`, `visible_cases_for`, `share_case`) all exist, are tested, and are the ground truth for everything else in this baseline. `RequirementAdjudication`, the promotion bridge, and the explicit Private→Shared Case transition are also now reachable through minimal route wiring in `routes/workspace.py` (`promote_requirement_item_route`, `adjudicate_requirement`, `share_case`).

## What's specified but unbuilt (fully designed, zero code)

Scenario/Scenario-Delta, `ViabilityAssessment`, `Template`, `ReferenceStandard`, Contract/Delivery DNA, Perspective (application layer), the graduated Contract Commitment event sequence, authoritative project metamorphosis (composes from existing primitives, no new object), Owner/Proponent publication (Published Procurement Instrument), the Neutral/Adjudication Workspace, the Experience Corpus (all forms), Project Security Policy (as governed `Requirement` content), the remainder of the investigation lifecycle extensions **beyond Case visibility itself** — collaboration threshold, Shared→Private retraction, Archive, `CaseLock`, Derive/Copy/Adopt, publication anchoring — Deficiency/completion-resolution, dormancy/restart. Full detail in `specified-unbuilt/`. (Case visibility — Private/Shared, the explicit Share transition — is now implemented; see the authorization table below and `current/kernel-object-model.md`.)
...
| Case visibility — PRIVATE / SHARED states | **IMPLEMENTED** — `CaseRecord.visibility`/`created_by`/`shared_by`/`shared_at`, `CaseWorkspaceStore.visible_cases_for` (the real enforcement point — every case listing/switching/default-selection query in `routes/workspace.py` goes through it), plus indirect-identifier guards on `artifact_image` and `preview_finding_id`. 24 tests in `tests/test_case_privacy.py`. |
| Explicit Case PRIVATE → SHARED transition | **IMPLEMENTED** — `CaseWorkspaceStore.share_case` (owner-only authority; no collaboration threshold exists yet, so no delegated/PM-override sharing authority is implemented), route wiring at `POST /projects/<project_id>/workspace/cases/<case_id>/share`. Covered by the same 24 tests above. |
| Case SHARED → COLLABORATIVE, the collaboration threshold, Shared→Private retraction, Archive, `CaseLock`, Derive/Copy/Adopt, publication anchoring | **NOT AUTHORIZED** — specified only; explicitly *not* implied by the two rows above. See `specified-unbuilt/investigation-lifecycle-extensions.md`. |
