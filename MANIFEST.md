...
| `routes/portal.py` | HTML page blueprint (`portal_bp`, no URL prefix). Routes: `/` (marketing home, public), `/health` (liveness/readiness probe, public), `/login` (GET form + POST handler, generic error message, same-site-only `?next=` redirect), `/logout`, `/upload` (`@login_required` — GET form + POST handler, accepts optional `actor`/`role` form fields), `/dashboard` and `/dashboard/<project_id>` (`@login_required` — demo data when no id given, real parsed document plus its governance audit trail otherwise). | Calls `services.auth.check_credentials`/`log_in`/`log_out`/`login_required`; `services.ingestion.ingest_upload` / `get_registry` / `get_governance_log`; reads `services.bhive_parser.REQUIREMENT_CATEGORIES`; renders `templates/index.html`, `login.html`, `upload.html`, `dashboard.html`; registered by `app.py`. |
...
| `templates/login.html` | Sign-in form (username/password, generic invalid-credentials message). | Extends `base.html`. Rendered by `routes/portal.py:login` (GET and the 401 error path). |
