...
| `routes/portal.py` | HTML page blueprint (`portal_bp`, no URL prefix). Routes: `/` (marketing home, public), `/health` (liveness/readiness probe, public), `/login` (GET form + POST handler, generic error message, same-site-only `?next=` redirect — defaults to `/gateway` when no `next` is given), `/logout`, `/gateway` (`@login_required` — post-login landing with two action cards: ingest vs. dashboard), `/upload` (`@login_required` — GET form + POST handler, accepts optional `actor`/`role` form fields), `/dashboard` and `/dashboard/<project_id>` (`@login_required` — demo data when no id given, real parsed document plus its governance audit trail otherwise). | Calls `services.auth.check_credentials`/`log_in`/`log_out`/`login_required`; `services.ingestion.ingest_upload` / `get_registry` / `get_governance_log`; reads `services.bhive_parser.REQUIREMENT_CATEGORIES`; renders `templates/index.html`, `login.html`, `gateway.html`, `upload.html`, `dashboard.html`; registered by `app.py`. |
...
| `services/auth.py` | Session-based login gate for the web UI: `check_credentials(username, password)` (single shared credential via `AUTH_USERNAME`/`AUTH_PASSWORD_HASH` — unconfigured means it always returns `False`, fails closed), `is_authenticated()`, `log_in`/`log_out` (Flask `session`), and the `@login_required` decorator (redirects to `/login?next=<path>`). Passwords compared with `werkzeug.security.check_password_hash`, never `==`. | Used by `routes/portal.py` (`login`/`logout` routes, `@login_required` on `upload`/`dashboard`) and `app.py`'s context processor (`is_authenticated()` → `authenticated` in every template). Reads `AUTH_USERNAME`/`AUTH_PASSWORD_HASH` from `config.py`. Does not touch `routes/api.py` — the JSON API is out of scope by design. |
...
| `templates/login.html` | Sign-in form (username/password, generic invalid-credentials message), styled as a centered `.gateway-card` — brand lockup, section label, footer telemetry (registry type, chassis name, `STATIC_VERSION`). | Extends `base.html`. Rendered by `routes/portal.py:login` (GET and the 401 error path). |
| `templates/gateway.html` | Post-login landing page: two `.gateway-action` cards ("Ingest a new document" / "View dashboard") instead of redirecting straight into one. Same `.gateway-card` shell as `login.html`. | Extends `base.html`. Rendered by `routes/portal.py:gateway`. Links to `portal.upload`/`portal.dashboard`. |
...
**Web UI login:**
`GET /upload` or `/dashboard` while unauthenticated → `@login_required` (`services/auth.py`) → redirect to `/login?next=<original path>` → `POST /login` with correct credentials → `services.auth.check_credentials` (`werkzeug.security.check_password_hash`) → `log_in()` sets the session → redirect to `next` (same-site paths only) if one was given, otherwise `/gateway` → the originally-requested page (or the gateway's two action cards). `/logout` clears the session and redirects to `/`. `routes/api.py` is unaffected throughout.
