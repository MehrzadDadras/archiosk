...
| `routes/api.py` | JSON API blueprint (`api_bp`, mounted at `/api/v1`). Routes: `POST /documents/ingest`, `GET /documents`, `GET /documents/<id>`, `GET /documents/<id>/requirements` (optional `?category=`), `GET /documents/<id>/milestones`, `GET /categories`. Handles `RequestEntityTooLarge` (413) for oversized uploads. | Calls `services.ingestion.ingest_upload` / `get_registry`; reads `services.bhive_parser.REQUIREMENT_CATEGORIES`; registered by `app.py`. |
...
| `services/bhive_parser.py` | The B-Hive core chassis: `BHiveParser` runs extract → segment → classify → assemble on an uploaded RFP/RFQ. Extract supports `.txt`/`.csv` (direct decode), `.docx` (`python-docx`), `.pdf` (`pypdf`). Classify uses the Anthropic API (batches of 25 chunks, per-batch timeout via `ANTHROPIC_TIMEOUT_SECONDS`, falls back to rule-based classification per-batch on `anthropic.APITimeoutError`, and for the whole document on any other exception) or a deterministic keyword classifier when no API key is set. Assemble derives a milestone list from `schedule_milestone`-category requirements. Defines `REQUIREMENT_CATEGORIES` and the `ParsedDocument`/`RequirementItem` dataclasses. | Used by `services/ingestion.py`. `ParsedDocument`/`RequirementItem` are consumed by `services/requirements_registry.py` for (de)serialization. `REQUIREMENT_CATEGORIES` is imported by both route blueprints. |
...
| `templates/dashboard.html` | The "Agility Engine" dashboard: header stats, honeycomb milestone lattice (`.hex` cells styled by status), requirement registry table with category filter chips. | Extends `base.html`. Rendered by `routes/portal.py:dashboard` with either demo data or a real `ParsedDocument`'s fields. Loads `static/js/dashboard.js` via `extra_scripts`. |
...
| File | Purpose | Connects to |
|---|---|---|
| `deploy/nginx.conf` | Reverse-proxy site config for archiosk.com. HTTP→HTTPS redirect, TLS server block, `/static/` alias with far-future caching, dedicated quiet `/health` location (tight timeouts, no access log) for external monitors, upstream passive health check (`max_fails=3 fail_timeout=30s`) against the single gunicorn backend. | Proxies to `127.0.0.1:8000`, matching `deploy/gunicorn.conf.py`'s default `bind`. The `/health` location targets `routes/portal.py`'s `/health` route. `client_max_body_size 25M` mirrors `MAX_UPLOAD_MB` in `config.py`. |
| `deploy/gunicorn.conf.py` | Gunicorn worker settings: bind address, worker count/class, timeouts, request-based worker recycling (`max_requests`/`max_requests_jitter`). | Loaded via `gunicorn -c deploy/gunicorn.conf.py wsgi:app`; `bind` must match what `deploy/nginx.conf`'s `upstream` proxies to. |
| `deploy/gunicorn.service` | systemd unit — runs gunicorn as the `archiosk` user/group, loads `.env` via `EnvironmentFile`, restarts on failure, applies sandboxing (`NoNewPrivileges`, `PrivateTmp`, `ProtectSystem`). | `ExecStart` points at `deploy/gunicorn.conf.py` and `wsgi:app`. |
