...
| `services/rfi_export.py` | `build_rfi_docx(document, operating_environment_label=None)` turns a `ParsedDocument`'s flagged `ConsistencyFlag`s into a real Request for Information `.docx` via `python-docx` — one `RFI-NNN` section per flag with both requirement excerpts and the explanation; stamps the optional CLAUDE-P29 `operating_environment_label` into the metadata block when given. `build_rfi_draft_docx(draft, operating_environment_label=None)` does the equivalent per-Finding governed export. Raises `RFIExportError` (message distinguishes "checked, nothing flagged" from "never checked") when there's nothing to export. | Called by `routes/api.py`'s `GET /documents/<id>/rfi` (unstamped — no `CaseWorkspaceStore` access there, a deliberate CLAUDE-P29 scope limitation) and `routes/workspace.py`'s `export_rfi`/`export_rfi_draft` (stamped). Consumes `ParsedDocument`/`ConsistencyFlag` from `services/bhive_parser.py`. |
