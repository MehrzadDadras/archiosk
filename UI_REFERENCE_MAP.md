...
| `lists.project.documents.send-to-eye` (CLAUDE-EYE-COMPARE-01; CLAUDE-LEFTRAIL-ROUTING-01 added toggle/active-state) | `<button aria-pressed>` (pattern, one per Document row, `data-source-id`/`data-source-name`/`data-source-kind`) | 👁 glyph only, `aria-label="Open <name> in Eye"` | Loads that Document into `eye.document-view` (`window.ArchioskEyePane.loadDocument`, `static/js/eye_pane.js`) — a pure client-side GET of the existing `workspace.source_file` route, never a navigation/mutation. Lets a Document be sent to Eye without replacing whatever is currently open in Display. `aria-pressed="true"` on whichever row's own document currently occupies Eye (restrained color/weight only — no border/box); clicking the ALREADY-active icon again clears Eye instead of reloading it | Same as workspace access | active |
