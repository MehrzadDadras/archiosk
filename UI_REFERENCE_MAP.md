...
| `chat.operational-actions` (CLAUDE-CA1D-RIVER-01, new) | `.conv-operational-actions` (pattern — `macros.operational_action_offers`, case-scoped and project-scoped conversation loops both use this same reference, rendered only on the most recent system message) | The "fourth beat" — real, already-implemented next actions ("Make a Task from this"/a contextual deadline-or-risk variant, "Highlight this answer") attached to `ConversationMessage.operational_actions`, offered ONLY when that reply's own `grounded_in` is genuinely non-empty (a real, evidence-backed answer, never a "not covered" one). Each is its own real `<form>` POSTing to the existing `create_task_route`/`add_tag_occurrence_route` with an anchor covering the whole answer message — no new backend mechanism. Capability-true by construction: only "task"/"tag" kinds are ever emitted, because Create Finding/Add to Risk Register/Delegate are not genuinely implemented as one-click conversational actions today | Same as workspace access | active |
| `chat.context-indicator` (CLAUDE-POSTCAMEL-CA1B, new) | `.conv-context-indicator` (pattern — rendered above both the case-scoped and project-level composer) | "Currently working with: Requirement/Finding/Source — {label}" plus a "Clear" button posting to `workspace.clear_selected_context` | Resolved read-time every render against the persisted `selected_object:{project_id}` session slot via `_resolve_anchor_object` — never shown for a stale/deleted selection | Same as workspace access | active |
