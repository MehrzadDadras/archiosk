...
| `landing.voice.button` (new, CLAUDE-CA1D-PUBLIC-LANDING-03) | `<button type="button">` | "🎙️ Speak to Archiosk" | Hidden server-side by default (`hidden` attribute); unhidden by `static/js/landing.js` only when `window.SpeechRecognition`/`webkitSpeechRecognition` is feature-detected — never a false promise on an unsupported browser. Push-to-talk (click to start, click again to stop) via the browser's own built-in Web Speech API — the exact same mechanism as the authenticated composer's own `chat.composer.voice` (CLAUDE-POSTCAMEL-VOICE1-PRE); no server round trip, no audio blob ever handled by this code | Unauthenticated visitors at `/` | active |
| `landing.voice.status` (new, CLAUDE-CA1D-PUBLIC-LANDING-03) | `<span aria-live="polite">` | (empty at rest) | "Listening…"/"Transcribing…" while active, or a specific failure (permission denied, no microphone, no speech detected, recognition unavailable, recognition stopped) — same `ERROR_MESSAGES` vocabulary as `chat.composer.voice.status` | Same as above | active |
| `landing.voice.result` (new, CLAUDE-CA1D-PUBLIC-LANDING-03) | `<div aria-live="polite">` | (hidden until a final transcript exists) | The transcript plus a small, deterministic, client-side-only keyword-routed response (never a generative/LLM call, never a network request) — a real link toward Explore/Request Trial Access/Sign In, or an honest "I'm not sure yet" fallback. Never higher than Level 2 (Suggest) of the future Voice authority ladder — see `governance/specified-unbuilt/voice-conversational-presence.md` | Same as above | active |
