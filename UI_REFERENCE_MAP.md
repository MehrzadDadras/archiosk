...
| `menu.account` | `<details>` popup | "…" (username) | Contains Sign out (first), then (admin-only) the Admin section — New Project/Security/Operations/Project Data Management — then Removed Projects and the UI Reference Mode toggle. **CLAUDE-LEFT-RAIL-01:** the Admin section and Removed Projects are relocated here from the left PROJECTS rail — see `menu.account.admin` below. **CLAUDE-APPEARANCE-SIMPLIFY-01:** Sign out moved here from the bottom of this same menu (Product Owner: "Sign out is an account/session action, not an Admin action") — always the first item, before the "Admin" label, so it reads unambiguously as account-level regardless of role | Every authenticated page | active |
| `menu.account.sign-out` (new, CLAUDE-APPEARANCE-SIMPLIFY-01) | `<a>` | "Sign out" | Navigates to `portal.logout` (same route/security behavior as before — a placement change only). First item in this menu, before the "Admin" label | Every authenticated page | active |
| `menu.account.admin` (new, CLAUDE-LEFT-RAIL-01) | `<span>` (non-interactive label) | "Admin" | Quiet section label distinguishing the relocated admin-only items from the account-level items below them — no divider rule, no new accent color, same restrained-landmark idiom `.current-project` already established | **Admin only** — `is_admin` | active |
| `menu.account.admin.new-project` (new, CLAUDE-LEFT-RAIL-01) | `<a>` | "+ New Project" | Navigates to `portal.upload`. Retired from `lists.new-project` — see "Retired references" below | **Admin only** — `is_admin` | active |
| `menu.account.admin.security` (new, CLAUDE-LEFT-RAIL-01) | `<a>` | "Security" | Navigates to `security.department_home`. Retired from `lists.security` — see "Retired references" below | **Admin only** — `is_admin` | active |
| `menu.account.admin.operations` (new, CLAUDE-LEFT-RAIL-01) | `<a>` | "Operations" | Navigates to `operations.department_home`. Retired from `lists.operations` — see "Retired references" below | **Admin only** — `is_admin` | active |
| `menu.account.admin.project-data-management` (new, CLAUDE-LEFT-RAIL-01; carries `?project_id=` as of CLAUDE-PROJECT-SURFACE-CONSOLIDATION-01) | `<a>` | "Project Data Management" | Navigates to `portal.reset_project_data`, now the real "Project Data Management" page (Add/Archive Documents for the active Project - see the `pdm.*` rows below), with the destructive whole-deployment reset demoted into a collapsed secondary disclosure rather than defining the page. Carries `?project_id=` when reached from an open Project's own page, so the destination can unmistakably identify which Project's evidence Add/Archive Documents acts on; omitted (renders an honest "no active project" state) when reached from the portfolio. Same `id="project-data-management"` DOM anchor preserved so "exactly one such element" stays a stable, checkable invariant | **Admin only** — `is_admin` | active |
...
| `lists.project.documents.image-search-status` (CLAUDE-DOCUMENT-RAIL-SEARCH-01, new) | `<p aria-live="polite">` | "Image/shape search is not yet available in ARCHIOSK — your query image is saved above, not discarded..." | The honest deferred state itself — shown once "Search" is clicked in Image mode, never a fabricated result list | Same as workspace access | active |
...
| `lists.project.documents.marked-filter` (CLAUDE-DOCUMENT-RAIL-PROBE-EYE-TOOL-01, new) | `<button aria-pressed>` | "Marked" | Compact toggle beside the Documents tree-toggle button — client-side row show/hide only (`static/js/document_marks.js`), no navigation, no separate panel. RAIL-SEARCH-01 (next stage) places the actual search field beside this same control | Same as workspace access | active |
...
| `display.files.data-room.reconcile.form` | `<form>` | — | POSTs to `reconcile_data_room_route` — real `webkitdirectory` folder-select reconciliation (Part 4): hash-identical files relink to their existing Source, genuinely new eligible files are ingested and registered, nothing already registered is ever removed | Admin only | active |
...
| `toolbox.composer-findings` | `<div>` (CLAUDE-GO-RIGHT-PANEL-01) | Rendered inside `toolbox.project-intelligence`, first, only when this project has at least one Composer-emitted `ComposerFinding` (`composer_findings_view`) — a compact "F-001 — tag" list, one row per finding | `composer_findings_view` is project-scoped (`workspace.composer_findings`), already isolated by `CaseWorkspaceStore`'s own per-project registry path | active |
...
| `spin.pane` (new, CLAUDE-SPIN-01 — converged from the retired variant-a pane ref) | `<section>` (`macros.tool_pane`) | "Spin — Evidence Isolation" — the one canonical Spin container | Same as workspace access | active |
...
| `spin.trigger` (new, CLAUDE-PROJECT-SURFACE-CONSOLIDATION-01) | `<button>` | "SPIN" | The single action that commits the current row selections as the active/engaged set — updates `spin.status`/`spin.summary`. Selecting a row alone never triggers this | Same as workspace access | active |
...
| `gateway.new-project` (new, CLAUDE-GO-NEUTRAL-ENTRY-01) | `<a>` | "New Project" | Navigates to plain `portal.upload` (no `?environment=` preset) — the form's own required "Project Operating Environment" radio + confirmation checkbox is the real commissioning step, unchanged; this link no longer pre-decides which side a project will be | **Admin only** — `is_admin` | active |
| `gateway.open-existing-projects` (new, CLAUDE-GO-NEUTRAL-ENTRY-01) | `<details>` | "Open Existing Project" | Reveals every authorized project INLINE on the Gateway (macros.subdisclosure), regardless of operating environment — one unfiltered `_environment_projects` call, the same access-scoped data `choose_project` itself builds, no new authorization surface. See `.search`/`.open-form`/`.leaf`/`.open-button`/`.empty` below | Every authenticated page | active |
| `gateway.open-existing-projects.search` (new, CLAUDE-GO-NEUTRAL-ENTRY-01) | `<input type=search>` | "Search your projects" | Client-side substring filter over the inline list below (`data-filter-target`) — no server round-trip, no new route | Same as parent | active |
| `gateway.open-existing-projects.open-form` (new, CLAUDE-GO-NEUTRAL-ENTRY-01) | `<form>` | (no visible label) | Same select-radio-then-submit pattern as `gateway.chooser.open-form` — wraps the radio group; submit rewrites to the selected Project's real URL client-side | Same as parent | active |
| `gateway.open-existing-projects.leaf` (new, CLAUDE-GO-NEUTRAL-ENTRY-01) | `<label>` (pattern) | a Project card, wrapping a radio | One row per Project this user can already open, any operating environment | Filtered to `_accessible_documents` (already access-scoped) | active |
| `gateway.open-existing-projects.open-button` (new, CLAUDE-GO-NEUTRAL-ENTRY-01) | `<button>` | "Open Project" | Disabled until a radio is checked; Enter from a checked radio submits the form natively | Same as parent | active |
| `gateway.open-existing-projects.empty` (new, CLAUDE-GO-NEUTRAL-ENTRY-01) | `<p>` | "No projects yet." | Zero-state when this user has no projects yet at all — the "New Project" action is already visible directly above, not duplicated here | Same as parent | active |
...
| `gateway.orientation.form` (new, CLAUDE-VOICE-CONSISTENCY-01) | `<form>` | "Ask Archiosk" (`aria-label` on the input) | A small, project-less orientation composer on `gateway.html` only (not `project_chooser.html`) — POSTs to `portal.gateway_orientation`, a new rule-based responder that never opens a `CaseWorkspaceStore`/calls `interpret_message` (never claims project truth — constitutional-invariants.md #8). Client-side fetch, JSON reply | Every authenticated page reaching `gateway.html` | active |
| `gateway.orientation.voice` (new, CLAUDE-VOICE-CONSISTENCY-01) | `<button type="button">` | "🎙️" (icon only) | Same shared engine (`static/js/voice_input.js`) and visual language as `chat.composer.voice` — fills `gateway.orientation.form`'s text input only, never auto-submits; the reviewer still clicks `gateway.orientation.submit` themselves | Same as parent | active |
| `gateway.orientation.voice.status` (new, CLAUDE-VOICE-CONSISTENCY-01) | `<span aria-live="polite">` | (empty at rest) | Same status vocabulary as `chat.composer.voice.status` | Same as parent | active |
| `gateway.orientation.submit` (new, CLAUDE-VOICE-CONSISTENCY-01) | `<button>` | "Ask" | Submits `gateway.orientation.form` | Same as parent | active |
| `gateway.orientation.reply` (new, CLAUDE-VOICE-CONSISTENCY-01) | `<p aria-live="polite">` | (hidden until a reply exists) | Renders `gateway_orientation()`'s response text; a "navigate" reply also redirects (e.g. to the matched Project or `portal.upload`) shortly after showing the confirmation text, same UX as `landing.voice.status`'s own direct-nav confirmation | Same as parent | active |
