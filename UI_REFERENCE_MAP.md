...
| `menu.account.admin.project-data-management` (new, CLAUDE-LEFT-RAIL-01) | `<a>` | "Project Data Management" | Navigates to `portal.reset_project_data`. Retired from `lists.system-data-management` — the destination page's own subtitle carries the "resets every Project in this deployment" warning directly, so a passing rail-side note was no longer the only place it appeared. Same `id="project-data-management"` DOM anchor preserved (was on a `macros.subdisclosure`, now on this plain link) so "exactly one such element" stays a stable, checkable invariant | **Admin only** — `is_admin` | active |
...
| `lists.project.tools` | tree-toggle `<button>` | "Project Tools" | Expands/collapses; contains every control below. **CLAUDE-P40-VW7B:** no longer contains Reset Project Data — see `menu.account.admin.project-data-management` above and "Retired references" below | Same as workspace access (individual controls below carry their own, narrower gates) | active |
...
| `spin.all-on`, `spin.all-off`, `spin.reset` (new, CLAUDE-SPIN-01 — converged from the retired variant-a equivalents) | `<button>` | "All ON" / "All OFF" / "Return to Baseline" — client-side prototype state only | Same as workspace access | active |
| `spin.toggle` (new, CLAUDE-SPIN-01 — converged from the retired variant-a toggle ref) | `<button aria-pressed>` (pattern, one per discipline) | "ON" / "OFF" — client-side per-discipline engagement toggle; rows never relocate | Same as workspace access | active |
...
| Reference | Element | Label | Current behavior | Auth notes | Status |
|---|---|---|---|---|---|
| `upload.limits` (CLAUDE-POSTCAMEL-META-T01, re-scoped; CLAUDE-HYBRID-ENTRY-01, wording corrected) | `<p>` | "Archiosk is designed to work alongside your project's existing material…" | States the CURRENT, honest behavior (both establishment paths below permanently upload to Archiosk's own server) rather than META-T01's original "your files stay wherever they already are" claim, which is no longer true of either mechanism — see `services/ingestion.py`'s own `ingest_upload`/`ingest_folder_upload`. Previously held the accepted-formats/size copy, now moved to `upload.limits.formats` below | `portal.upload` — **admin only** | active |
...
| `upload.submit` | `<button>` | "Create project and parse document" | Submits the upload form via its default action (`portal.upload`). Neither this path nor the folder path below is framed as "recommended" as of CLAUDE-HYBRID-ENTRY-01 — both are honest, equally-weighted permanent-upload paths pending a real linked-storage mechanism | Same as above | active |
| `upload.folder.picker-button` (new, CLAUDE-CA1D-RECEPTION-FIX-01; relabeled CLAUDE-HYBRID-ENTRY-01) | `<button>` | "Choose Folder to Upload…" | Opens the hidden `webkitdirectory` file input. Was "Choose Project Folder…" — corrected because a real reviewer read that wording as folder-linking, not upload | `portal.upload` — **admin only** | active |
...
| `upload.folder.submit` (new, CLAUDE-CA1D-RECEPTION-FIX-01; relabeled + demoted CLAUDE-HYBRID-ENTRY-01) | `<button>` | "Upload Folder Contents (Legacy)" | Submits via `formaction` to `portal.upload_folder` — disabled until a founding document is confirmed. Was "Establish Project from Folder" / `btn-primary`; relabeled and demoted to `btn-ghost` so this legacy, full-server-upload path no longer reads as the recommended or as a folder-linking mechanism (Product Owner-reported live confusion, CLAUDE-HYBRID-ENTRY-01) | Same as above | active |
