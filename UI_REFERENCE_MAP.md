...
| `menu.account` | `<details>` popup | "…" (username) | Contains (admin-only) the Admin section — New Project/Security/Operations/Project Data Management — plus Removed Projects, UI Reference Mode toggle, and Sign out. **CLAUDE-LEFT-RAIL-01:** the Admin section and Removed Projects are new here, relocated from the left PROJECTS rail — see `menu.account.admin` below | Every authenticated page | active |
