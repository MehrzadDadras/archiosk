...
| `gateway.open-existing-client-owner` (behavior changed, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<details>` (was `<a>`) | "Open Existing Project" (inside the "Client / Owner Projects" context group) | Reveals that group's own Projects INLINE on the Gateway (macros.subdisclosure) — no longer navigates to `portal.choose_project`. Fewest-transitions reopen path: Gateway → reveal → select → workspace. See `.search`/`.open-form`/`.leaf`/`.open-button`/`.empty` below | Every authenticated page | active |
| `gateway.open-existing-design-builder` (behavior changed, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<details>` (was `<a>`) | "Open Existing Project" (inside the "Design-Builder / Proponent Projects" context group) | Same inline-reveal behavior, scoped to Design-Builder/Proponent Projects | Every authenticated page | active |
| `gateway.open-existing-client-owner.search` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<input type=search>` | "Search your Client / Owner projects" | Client-side substring filter over the inline list below (`data-filter-target`) — no server round-trip, no new route | Same as parent | active |
| `gateway.open-existing-client-owner.open-form` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<form>` | (no visible label) | Same select-radio-then-submit pattern as `gateway.chooser.open-form` — wraps the radio group; submit rewrites to the selected Project's real URL client-side | Same as parent | active |
| `gateway.open-existing-client-owner.leaf` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<label>` (pattern) | a Project card, wrapping a radio | One row per Client/Owner Project this user can already open — same `_environment_projects` data `choose_project` itself builds, no new authorization surface | Filtered to `_accessible_documents` (already access-scoped) | active |
| `gateway.open-existing-client-owner.open-button` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<button>` | "Open Project" | Disabled until a radio is checked; Enter from a checked radio submits the form natively | Same as parent | active |
| `gateway.open-existing-client-owner.empty` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<p>` | "No Client / Owner projects yet." | Zero-state when this user has no Client/Owner Projects yet — the "New Project" action is already visible directly above in the same context group, not duplicated here | Same as parent | active |
| `gateway.open-existing-design-builder.search` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<input type=search>` | "Search your Design-Builder / Proponent projects" | Same client-side filter, scoped to this group's own list | Same as parent | active |
| `gateway.open-existing-design-builder.open-form` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<form>` | (no visible label) | Same select-radio-then-submit pattern | Same as parent | active |
| `gateway.open-existing-design-builder.leaf` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<label>` (pattern) | a Project card, wrapping a radio | One row per Design-Builder/Proponent Project this user can already open | Filtered to `_accessible_documents` (already access-scoped) | active |
| `gateway.open-existing-design-builder.open-button` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<button>` | "Open Project" | Disabled until a radio is checked | Same as parent | active |
| `gateway.open-existing-design-builder.empty` (new, CLAUDE-CA1D-GATEWAY-INLINE-REOPEN-01) | `<p>` | "No Design-Builder / Proponent projects yet." | Zero-state when this user has no Design-Builder/Proponent Projects yet | Same as parent | active |
