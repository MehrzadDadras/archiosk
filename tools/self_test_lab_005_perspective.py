...
def main() -> int:
    ...
    try:
        ...
        question = (
            "How should this be understood in terms of risk and opportunity for the "
            "party I represent?"
        )
        ...
        if not req(corpus["risk_transfer_id"]):
            print("SKIPPED: corpus construction failed.")
            return 1
        ...
        print(f"PASS" if ok_a else "FAIL", "- DB must be 'risk', Owner must not be 'risk'.")
        all_ok &= ok_a
        # Same canonical clause cited on both sides - never rewritten per party.
        ...
        # Reuse Case A's real, blind Design-Builder machine result for the
        # CONVERGENCE half - the machine already ran blind above; we now
        # record a human perspective assessment AFTER reading its real
        # output, deliberately agreeing with it, to construct a genuine
        # convergent pair (not to influence the machine, which cannot see
        # this human record and never will for this call).
        ...
        # Reuse Case C's real, blind Design-Builder machine result for the
        # DISAGREEMENT half - deliberately record a human override with a
        # DIFFERENT polarity than the machine's real, already-produced
        # answer, representing a genuine reviewer disagreement.
