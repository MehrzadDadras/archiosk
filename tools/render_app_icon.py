...
CLAUDE-MARK-RESTORE-01 (2026-09-06): this geometry is RESTORED, not new. It is
CLAUDE-MOBILE-ICON-01's constructed mark, recovered verbatim from 9d16b8c^ after
the Product Owner identified the letterform that replaced it as a substitute
rather than a decision.

WHAT HAPPENED, KEPT RATHER THAN TIDIED AWAY.

9d16b8c (CLAUDE-LETTERMARK-PURGE-01, 2026-08-30) did two separable things: it
removed the symbol from UI chrome, and it replaced this geometry with a letter
A. The first rests on real evidence recorded in that commit - the mark read as a
BOWTIE at 16px beside the app menu and at 36px on the sign-in card, and as an
HOURGLASS in a browser tab. That half STANDS and is not touched here: chrome is
still wordmark-only.

The second half did not follow from that evidence. The failures were at 16-36px;
the verdict was applied at every size, including the landing hero (38-64px) and
the home-screen rasters (180-512px), where no failure was ever recorded. It also
reinstated a form the Product Owner had already rejected by name: 98086b6
(CLAUDE-BOTTLENECK-ADOPTION-01, 2026-08-13) records the direction "stop
presenting the generic yellow-square 'A' as ARCHIOSK's primary mark". 9d16b8c
named that commit as reversed but described what the A had replaced only as "a
bee-yellow badge" - it never surfaced that the new icon was the rejected
letterform returning. That omission is why the substitute read as a decision.

The pipeline below is unchanged by the restore and was never the problem:
ICO_SIZES (16/32/48/64, each rendered at its own size), the --proof sheet at
true home-screen sizes, and the single-geometry-feeds-every-raster property all
predate the purge and survive it.

THE SMALL-SIZE PROBLEM IS REAL AND IS NOT SOLVED BY THIS FILE.

Restoring the mark restores its 16px behaviour too. The mitigation is where the
mark is USED, not how it is drawn: it appears only where an icon is genuinely
required - the landing hero, the tab, the home screen - and never beside a small
chrome label, which is exactly what half one of the purge established. If the
tab icon proves unacceptable again at 16px, the honest next step is a
size-specific frame in the .ico, not a second mark.
