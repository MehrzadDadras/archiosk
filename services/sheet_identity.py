...
def recovered_text_for(workspace, source_id: str) -> str:
    ...
    Reads what the perception worker already wrote - positioned lines first,
    falling back to the unpositioned text evidence for a Source perceived
    before coordinates existed. Nothing is re-extracted and no image is opened.
    """
    ...
    for item in (getattr(workspace, "evidence_items", None) or []):
        ...
        if item.get("content_type") == positioned_text.POSITIONED_CONTENT_TYPE:
            ...
            if region is None or unit is None or unit.get("source_id") != source_id:
                continue
            address = region.get("address") or {}
            positioned.append(((unit.get("order_index") or 0),
                               address.get("y", 0.0), address.get("x", 0.0), content))
        elif item.get("content_type") == "text":
            plain.append(content)

    if positioned:
        positioned.sort(key=lambda row: row[:3])
        return "\n".join(row[3] for row in positioned)
    return "\n".join(plain)
...
def register_sheet_index(store, workspace, source_id: str, *,
    ...
    text = recovered_text_for(workspace, source_id)
    entries = index_entries(text)
    report["is_index"] = looks_like_index(text)
