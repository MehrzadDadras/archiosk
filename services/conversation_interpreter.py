...
from services import drawing_analysis
...
def _handle_analyze(
    ...
    # CLAUDE-ANALYZE-BOUNDARY-01. THE ONE DECISION THIS FUNCTION MAKES BEFORE IT
    # ANALYSES ANYTHING: what kind of engine may serve this request.
    #
    # There is no real drawing-analysis engine behind this intent today. The
    # deployed perception stack (services/perception_worker.py and the legend
    # chain) reads positioned TEXT from a sheet; it does not produce candidate
    # findings about a drawing against a stated objective, which is what Analyze
    # asks for. That gap is real, and it is reported rather than papered over -
    # inventing the integration was explicitly out of scope for this tranche.
    #
    # So the honest states are exactly two: a labelled prototype, or nothing.
    # What must never happen is the third thing - canned output presented as a
    # reading of the drawing.
    capability = drawing_analysis.analysis_capability()
    if capability != drawing_analysis.CAPABILITY_PROTOTYPE:
        return InterpretationResult(
            action_taken="analyze_unavailable",
            reply_text=(
                "No drawing-analysis engine is available for this Investigation. "
                "The prototype analysis is switched off, and no production "
                "perception engine serves this request yet, so there is nothing "
                "honest to return. Nothing was generated and nothing was stored."
            ),
        )
    ...
    return InterpretationResult(
        ...
        reply_text=(
            f"PROTOTYPE ANALYSIS on \"{source['name']}\" — this did NOT read the "
            f"drawing. {count} illustrative candidate finding(s) were generated by "
            f"{ENGINE_NAME} {ENGINE_VERSION}, each cropped to a fixed region of the "
            f"sheet. The crops are real pixels; the finding text is not a visual "
            f"inference and carries no authority. Treat every one as a prompt for "
            f"your own review, not as something ARCHIOSK found — see the Artifact "
            f"Workspace.{context_note}"
        ),
    )
