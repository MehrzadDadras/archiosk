...
def developer_home_composer():
    ...
    # GOPILOT NERVOUS SYSTEM: the ONE governed image intake, with this project-less
    # surface's own external-AI gate. Nothing is persisted - orientation only.
...
@portal_bp.route('/admin/survey-evaluation/<run_id>/source-review', methods=['GET', 'POST'])
@admin_required
def evaluation_source_review(run_id):
    ...
    try:
        ...
        if request.method == 'POST':
            try:
                with evaluation.isolated(current_app, path):
                    dx.apply_source_review_action(store, workspace, source_id, request.form,
                        session.get('username'), allowed_root=path)
                flash('Evaluation review recorded. Consumer re-evaluation is explicit.', 'success')
            except (ValueError, OSError, CaseWorkspaceError) as exc:
                flash(str(exc), 'error')
            return redirect(url_for('portal.evaluation_source_review', run_id=run_id))
...
@portal_bp.route('/admin/survey-evaluation/<run_id>/review-image/<kind>')
@admin_required
def evaluation_review_image(run_id, kind):
    _require_developer_tools()
    from services import survey_evaluation as evaluation, document_examination as dx, image_intake
    ...
    try:
        ...
        if kind == 'original' and filename.lower().endswith('.pdf'):
            # Retained evaluation PDF raster, generated by the explicit create
            # action. Serving it does not rasterize or analyze on GET.
            raw = (path/'source.png').read_bytes()
            filename = 'source.png'
        verdict = image_intake.verify_image_bytes(raw, filename)
        if verdict['status'] != image_intake.VERIFIED:
            abort(404)
        return send_file(io.BytesIO(raw), mimetype='image/png' if raw.startswith(b'\x89PNG') else 'image/jpeg')
